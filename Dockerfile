...
RUN mv docker/fly/supervisor.conf /etc/supervisord.conf
RUN mv docker/fly/nginx.conf /etc/nginx/nginx.conf
RUN mv docker/fly/php.ini /etc/php8/conf.d/php.ini
RUN mv docker/fly/php-fpm.conf /etc/php8/php-fpm.conf
RUN mv docker/fly/app.conf /etc/php8/php-fpm.d/app.conf

# mix assets (js/css)
RUN npm install && npx mix
